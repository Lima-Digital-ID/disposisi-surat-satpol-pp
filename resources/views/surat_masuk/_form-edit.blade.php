    ...
    <div class="form-group row">
        ...
        <div class="col-sm-10">
            <select name="sifat_surat" id="sifat_surat" class="form-control @error('sifat_surat') is-invalid @enderror">
                ...
                <option value="Biasa" {{ old('sifat_surat', $data->sifat_surat) == 'Biasa' ? ' selected' : '' }}>Biasa
                </option>
                <option value="Pribadi" {{ old('sifat_surat', $data->sifat_surat) == 'Pribadi' ? ' selected' : '' }}>
                    Pribadi
                </option>
            </select>
            @error('sifat_surat')
            ...
            <select name="pengirim" id="pengirim" class="js-example-tags" style="width: 100%;" required>
                ...
                @foreach ($allPengirim as $sender)
                    <option value="{{ $sender->id }}"
                        {{ old('id_pengirim', $data->id_pengirim) == $sender->id ? ' selected' : '' }}>
                        {{ $sender->pengirim }}</option>
                @endforeach
            </select>
            @error('pengirim')
                <div class="invalid-feedback">
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-10">
            <input type="date" name="tgl_pengirim" class="form-control @error('tgl_pengirim') is-invalid @enderror"
                placeholder="Tanggal Kirim" value="{{ old('tgl_pengirim', $data->tgl_pengirim) }}">
            @error('tgl_pengirim')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-10">
            <input type="date" name="tgl_penerima" class="form-control @error('tgl_penerima') is-invalid @enderror"
                placeholder="Tanggal Kirim" value="{{ old('tgl_penerima', $data->tgl_penerima) }}">
            @error('tgl_penerima')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-10">
            <select name="tembusan" id="tembusan" class="form-control @error('tembusan') is-invalid @enderror">
                ...
                <option value="Langsung" {{ old('tembusan', $data->tembusan) == 'Langsung' ? ' selected' : '' }}>
                    Langsung</option>
                <option value="Tembusan" {{ old('tembusan', $data->tembusan) == 'Tembusan' ? ' selected' : '' }}>
                    Tembusan
                </option>
            </select>
            @error('tembusan')
